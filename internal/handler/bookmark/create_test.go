package bookmark_handler

import (
	"bytes"
	"encoding/json"
	"net/http"
	"net/http/httptest"
	"net/url"
	"testing"

	"github.com/gin-gonic/gin"
	"github.com/go-playground/validator/v10"
	jwt "github.com/golang-jwt/jwt/v5"
	bookmark_model "github.com/homework/lab/internal/models/dto/api/bookmark"
	bookmark_service_mocks "github.com/homework/lab/internal/service/bookmark/mocks"
	"github.com/homework/lab/pkg/request_ultils"
	"github.com/homework/lab/pkg/response"
	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/mock"
)

func init() {
	request_ultils.InputValidator.RegisterValidation("url", func(fl validator.FieldLevel) bool {
		value, ok := fl.Field().Interface().(string)
		if !ok || value == "" {
			return false
		}
		_, err := url.ParseRequestURI(value)
		return err == nil
	})
	request_ultils.InputValidator.RegisterValidation("url;required", func(fl validator.FieldLevel) bool {
		value, ok := fl.Field().Interface().(string)
		if !ok || value == "" {
			return false
		}
		_, err := url.ParseRequestURI(value)
		return err == nil
	})
}

func TestBookmarkHandler_CreateBookmark(t *testing.T) {
	testCases := []struct {
		name         string
		setupMock    func(ctx *gin.Context, input *bookmark_model.NewBookmarkRequest) *bookmark_service_mocks.BookmarkService
		body         string
		withClaims   bool
		expectedCode int
		expectedText string
	}{
		{
			name: "success",
			setupMock: func(ctx *gin.Context, input *bookmark_model.NewBookmarkRequest) *bookmark_service_mocks.BookmarkService {
				mockSvc := bookmark_service_mocks.NewBookmarkService(t)
				mockSvc.On("NewBookmark", ctx, "user-1", mock.MatchedBy(func(req *bookmark_model.NewBookmarkRequest) bool {
					return req != nil && req.Url == "https://example.com" && req.Description == "demo"
				})).Return(&bookmark_model.BookmarkInfo{
					Id:          "b-1",
					Code:        "ABC123",
					Url:         "https://example.com",
					Description: "demo",
				}, nil)
				return mockSvc
			},
			body:         `{"url":"https://example.com","description":"demo"}`,
			withClaims:   true,
			expectedCode: http.StatusOK,
			expectedText: SuccessCreateBookmark,
		},
		{
			name: "missing claims",
			setupMock: func(ctx *gin.Context, input *bookmark_model.NewBookmarkRequest) *bookmark_service_mocks.BookmarkService {
				return bookmark_service_mocks.NewBookmarkService(t)
			},
			body:         `{"url":"https://example.com","description":"demo"}`,
			withClaims:   false,
			expectedCode: http.StatusUnauthorized,
			expectedText: "Claims extract token",
		},
		{
			name: "invalid request",
			setupMock: func(ctx *gin.Context, input *bookmark_model.NewBookmarkRequest) *bookmark_service_mocks.BookmarkService {
				return bookmark_service_mocks.NewBookmarkService(t)
			},
			body:         `{"url":"not a url"}`,
			withClaims:   true,
			expectedCode: http.StatusBadRequest,
			expectedText: "Url",
		},
		{
			name: "service error",
			setupMock: func(ctx *gin.Context, input *bookmark_model.NewBookmarkRequest) *bookmark_service_mocks.BookmarkService {
				mockSvc := bookmark_service_mocks.NewBookmarkService(t)
				mockSvc.On("NewBookmark", ctx, "user-1", mock.MatchedBy(func(req *bookmark_model.NewBookmarkRequest) bool {
					return req != nil && req.Url == "https://example.com" && req.Description == "demo"
				})).Return((*bookmark_model.BookmarkInfo)(nil), response.InternalError)
				return mockSvc
			},
			body:         `{"url":"https://example.com","description":"demo"}`,
			withClaims:   true,
			expectedCode: http.StatusInternalServerError,
			expectedText: response.InternalError.Error(),
		},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			rec := httptest.NewRecorder()
			ctx, _ := gin.CreateTestContext(rec)
			ctx.Request = httptest.NewRequest(http.MethodPost, "/v1/bookmarks", bytes.NewBufferString(tc.body))
			ctx.Request.Header.Set("Content-Type", "application/json")
			if tc.withClaims {
				ctx.Set("claims", jwt.MapClaims{"sub": "user-1"})
			}

			var input *bookmark_model.NewBookmarkRequest
			if tc.body != "" {
				input = &bookmark_model.NewBookmarkRequest{}
				if err := json.NewDecoder(bytes.NewBufferString(tc.body)).Decode(input); err != nil && tc.expectedCode != http.StatusBadRequest {
					t.Fatal(err)
				}
			}
			service := tc.setupMock(ctx, input)
			handler := NewBookmarkHandler(service)
			handler.CreateBookmark(ctx)

			assert.Equal(t, tc.expectedCode, rec.Code)
			assert.Contains(t, rec.Body.String(), tc.expectedText)
		})
	}
}

